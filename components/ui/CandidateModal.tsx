...
export default function CandidateModal({name, recipient_email, position, applicationId, invite, summary} : {name: string, recipient_email: string, position: string, applicationId: number, invite: string, summary: string}) {
  ...
  const handleOnClick = async () => {
    const url_path = Math.random().toString().slice(2, 12);
    console.log(url_path);
    try {
        const response = await fetch('/api/invitation', {
            method: 'POST',
            body: JSON.stringify({ url_path: url_path, id: applicationId, recipient_email: recipient_email}),
        })
              ...
              <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                ...
                <div className="relative p-6 flex-auto">
                  <p className="my-4 text-blueGray-500 text-lg leading-relaxed">
                    {`Do you want to send an invite link to ${name} for ${position}?`}
                  </p>
                </div>
                {/*footer*/}
